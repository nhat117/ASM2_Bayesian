...
genMCMC = function( data , xName="x" , yName="y" , 
  ...
  model {
  for ( i in 1:Ntotal ) {
    zy[i] ~ dt( zbeta0 + zbeta1 * zx[i] + zbeta2 * zx[i]^2, 1/zsigma^2 , nu )
  }
  ...
  beta1 <- zbeta1 * ysd / xsd - 2 * zbeta2 * xm * ysd / (xsd^2)
  beta0 <- zbeta0 * ysd + ym - zbeta1 * xm * ysd / xsd + zbeta2 * xm^2 * ysd / (xsd^2)
  sigma <- zsigma * ysd
  }
  ...
  parameters = c( "beta0" ,  "beta1" , "beta2" , "sigma", 
                  "zbeta0" , "zbeta1" , "zbeta2", "zsigma", "nu" )
...
plotMCMC = function( codaSamples , data , xName="x" , yName="y" ,
  ...
  if ( pairsPlot ) {
    ...
    pairs( cbind( beta0 , beta1 , beta2 , sigma , log10nu )[plotIdx,] ,
  ...
  histInfo = plotPost( beta2 , cex.lab = 1.75 , showCurve=showCurve ,
                       ...
                       xlab=bquote(beta[2]) , main=paste("Slope") )
  ...
  for ( i in floor(seq(from=1,to=chainLength,length=nPredCurves)) ) {
    lines( xComb , beta0[i] + beta1[i]*xComb + beta2[i]*xComb^2, col="skyblue" )
  }
  ...
  for ( i in floor(seq(from=1,to=chainLength,length=nPredCurves)) ) {
    for ( j in 1:length(curveXpos) ) {
      ...
      yPred = beta0[i] + beta1[i]*curveXpos[j] + beta2[i]*curveXpos[j]^2 
  ...
  if ( showIntercept ) {
    ...
    plot( x , y , cex=1.5 , lwd=2 , col="black" , xlim=xLim , ylim=yLim ,
          xlab=xName , ylab=yName , cex.lab=1.5 ,
          main=paste( "Data w. Post. Pred. & ",postPredHDImass*100,"% HDI showing intercept" ,sep="") , 
          cex.main=1.33  )
